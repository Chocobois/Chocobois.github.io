...
export default function MDXProviderChoco({ children }: CustomMDXProvider) {
    return (<>
        <MDXProvider components={{
                code: ({className, children, ...props}) => {
                    ...
                    const split = String(children).trim().split(regexpAt);
                ...
                pre: ({children, ...props}) => <pre className='m-0 p-1' {...props}>{children}</pre>
            }}>
            {children}
        </MDXProvider>
    </>);
